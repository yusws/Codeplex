...
{
    class SqlSiteRoleProvider : SqlRoleProvider
    {
        #region Member Data
        ...
        public override string ApplicationName
        {
            get
            {
                ...
                {
                    ULS.LogError("SqlSiteRoleProvider.ApplicationName", "Connection String is empty or null", "RoleProvider");
                    throw new ProviderException("Connection string not defined for membership db.");
                }

                if (string.IsNullOrEmpty(appName))
                {
                    ULS.LogError("SqlSiteRoleProvider.ApplicationName", "No AppName retrieved from mapping", "RoleProvider");
        ...
        private string GetMappedAppName(string fqdn, string connString)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(appName))
                    ULS.LogMessage("SqlSiteRoleProvider.GetMappedAppName", "Could Not Get Mapped Application Name for FQDN: " + fqdn, "RoleProvider", TraceProvider.TraceSeverity.CriticalEvent);
            }
            finally
            {
